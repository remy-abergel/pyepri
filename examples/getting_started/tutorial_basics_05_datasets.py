...
# %%
#
# Before presenting each dataset, let us prepare the data loading
# process.
#
...
# computation time in practical applications.
#
...
# Bruker spectrometer. It comprised of two files in ``.pkl`` format:
#
# + ``phalanx-20220203-h.pkl``: contains the reference spectrum (1D
#   spectrum) of the sample and the associated homogeneous magnetic
#   field intensity (sampling nodes);
#
# + ``phalanx-20220203-proj.pkl``: contains the 2D projections (= 2D
#   sinogram), the associated homogeneous magnetic field intensity
#   nodes and the corresponding field gradient vectors coordinates.
#
...
#--------------#
dtype = 'float32'
path_proj = datasets.get_path('phalanx-20220203-proj.pkl') # returns the location of this dataset on your system
path_h = datasets.get_path('phalanx-20220203-h.pkl') # returns the location of this dataset on your system
...
# ``phalanx-20220203-proj.DSC``). See :ref:`here
# <tutorial_data_loading>` how to deal with those kinds of files.
#
...
# |
#
# The dataset is comprised of ``.pkl`` files stored using the same
# suffix naming conventions as the other datasets
# (``datasetname-{proj,h}.npy``). It contains 94 projections acquired
# with constant field gradient magnitude equal to 57.6 G/cm and
# orientation uniformly sampled between -90° and 90°. Each projection
# contains 1300 measurement points (same for the reference
...
path_proj = datasets.get_path(datasetname + '-proj.pkl') # returns the location of this dataset on your system
path_h = datasets.get_path(datasetname + '-h.pkl') # returns the location of this dataset on your system
dataset_proj = io.load(path_proj, backend=backend, dtype=dtype) # load the dataset containing the projections
dataset_h = io.load(path_h, backend=backend, dtype=dtype) # load the dataset containing the reference spectrum
B = dataset_proj['B'] # get B nodes from the loaded dataset
proj = dataset_proj['DAT'] # get projections data from the loaded dataset
...
path_proj = datasets.get_path(datasetname + '-proj.pkl')
path_hmixt = datasets.get_path(datasetname + '-hmixt.pkl')
path_htam = datasets.get_path(datasetname + '-htam.pkl')
path_htempo = datasets.get_path(datasetname + '-htempo.pkl')
dataset_proj = io.load(path_proj, backend=backend, dtype=dtype) # load the dataset containing the projections
dataset_htam = io.load(path_htam, backend=backend, dtype=dtype) # load the dataset containing the reference spectrum of the TAM
dataset_htempo = io.load(path_htempo, backend=backend, dtype=dtype) # load the dataset containing the reference spectrum of the TEMPO
dataset_hmixt = io.load(path_hmixt, backend=backend, dtype=dtype) # load the dataset containing the reference spectrum of the whole sample (TAM + TEMPO)
B = dataset_proj['B'] # get B nodes from the loaded dataset
proj = dataset_proj['DAT'] # get projections data from the loaded dataset
fgrad = dataset_proj['FGRAD'] # get field gradient data from the loaded dataset
htam = dataset_htam['DAT'] # get TAM reference spectrum data from the loaded dataset
htempo = dataset_htempo['DAT'] # get TEMPO reference spectrum data from the loaded dataset
hmixt = dataset_htempo['DAT'] # get the TAM + TEMPO reference spectrum data from the loaded dataset (also equal to htam + htempo)
