...
def test_4d_toeplitz_kernel(libname, dtype, nruns, tol):
    ...
    for id in range(nruns):
        ...
        phi = ss.compute_4d_toeplitz_kernel(B, delta, fgrad, (2*Nb, 2*N1, 2*N2, 2*N3), backend=backend, eps=eps, rfft_mode=rfft_mode, nodes=nodes) # fixed since cufinufft 3.4.0
